 ...
 * While locked, categories that already have a row for the month are skipped
 * rather than overwritten; the count is returned as `skipped`.
 */
export default async function handler(req, res) {
  ...
  if (value.items.length === 0) {
    res.status(200).json({ success: true, updated: 0, merged: 0, skipped: 0, month: value.month });
    return;
  }
